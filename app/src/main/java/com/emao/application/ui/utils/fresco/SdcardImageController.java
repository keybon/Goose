...
public class SdcardImageController {
    ...
    public static class SdcardControllerBuilder {
        ...
        public SdcardControllerBuilder setPlaceDrawable(int placeId) {
            sdcardParams.placeDrawable = MainApp.IMApp.getResources().getDrawable(placeId);
            return this;
        }
        ...
        public SdcardControllerBuilder setFailDrawable(int failId) {
            sdcardParams.failDrawable = MainApp.IMApp.getResources().getDrawable(failId);
            return this;
        }
         ...
         * @return
         */
        public SdcardControllerBuilder setClearCahe(boolean clearCahe) {
            sdcardParams.clearCahe = clearCahe;
            return this;
        }
    ...
    /**
     * 强制更新后本地同样名字，但是不同的图片，清除某条缓存
     *
     * @param uri
     */
    public void clearCacheImage(Uri uri) {
...
//            throw new NullPointerException("展示本地图片时sdcard路径不允许为空");
        ...
        if(this.clearCahe){
            clearCacheImage(Uri.parse("file://" + this.path));
        }
